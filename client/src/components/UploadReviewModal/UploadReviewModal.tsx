    ...
    <div className="review-overlay" onClick={onClose}>
      <div className="review-modal" onClick={(e) => e.stopPropagation()}>
        <div className="review-header">
          ...
          <button className="review-close-btn" onClick={onClose} disabled={loading}><FiX size={22} /></button>
        </div>

        <div className="review-summary">
          <div className="summary-item"><span className="summary-label">Transactions</span><span className="summary-value">{summary.count}</span></div>
          <div className="summary-item credit"><span className="summary-label">Total Credits</span><span className="summary-value">+₹{summary.totalCredits.toLocaleString()}</span></div>
          <div className="summary-item debit"><span className="summary-label">Total Debits</span><span className="summary-value">-₹{summary.totalDebits.toLocaleString()}</span></div>
        </div>

        <div className="review-table-wrapper">
          {transactions.length === 0 ? (
            <div className="review-empty"><p>All transactions have been removed.</p></div>
          ) : (
            <table className="review-table">
              <thead>
                <tr><th>#</th><th>Date</th><th>Description</th><th>Category</th><th>Type</th><th>Amount (₹)</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {transactions.map((t, index) => (
                  editingIndex === index ? (
                    <tr key={index} className="editing-row">
                      <td>{index + 1}</td>
                      <td><input type="date" className="review-input" value={editData.date || ''} onChange={(e) => setEditData(d => ({ ...d, date: e.target.value }))} /></td>
                      <td><input type="text" className="review-input" value={editData.description || ''} onChange={(e) => setEditData(d => ({ ...d, description: e.target.value }))} /></td>
                      <td>
                        <select className="review-input" value={editData.category || ''} onChange={(e) => setEditData(d => ({ ...d, category: e.target.value }))}>
                          {CATEGORIES[editData.type || 'debit'].map(c => <option key={c} value={c}>{c}</option>)}
                        </select>
                      </td>
                      <td>
                        <select className="review-input" value={editData.type || 'debit'} onChange={(e) => setEditData(d => ({ ...d, type: e.target.value as 'credit' | 'debit' }))}>
                          <option value="debit">Expense</option>
                          <option value="credit">Income</option>
                        </select>
                      </td>
                      <td><input type="number" className="review-input amount-input" value={editData.amount || ''} step="0.01" min="0" onChange={(e) => setEditData(d => ({ ...d, amount: e.target.value as any }))} /></td>
                      <td>
                    ...
                    <tr key={index}>
                      ...
                      <td className={`amount-cell ${t.type}`}>{t.type === 'credit' ? '+' : '-'}₹{Math.abs(t.amount).toLocaleString()}</td>
                      <td>
                        <div className="row-actions">
        ...
        <div className="review-footer">
          <button className="review-cancel-btn" onClick={onClose} disabled={loading}>Cancel</button>
          <button className="review-confirm-btn" onClick={handleConfirm} disabled={loading || transactions.length === 0}>
